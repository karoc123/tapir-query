...
use crate::engine::{
    ColumnSchema, CsvQueryEngine, EngineResult, ExportResult, QueryChunk, RegisteredCsv,
};
...
impl DuckDbEngine {
        ...
        F: FnOnce(&Connection) -> EngineResult<T>,
    {
        ...
        for table in registered.values() {
            ...
            connection.execute_batch(&register_sql).map_err(|error| {
                ...
                AppError::Sql(format!("failed to register CSV view {}: {error}", table.table_name))
            })?;
        }
    ...
    fn derive_table_name(
        ...
        let file_stem = file_path
            ...
            .ok_or_else(|| {
                AppError::Validation(String::from(
                    "unable to derive a table name from file path",
                ))
            })?;
    ...
    fn describe_query_schema(&self, connection: &Connection, sql: &str) -> EngineResult<Vec<ColumnSchema>> {
...
mod tests {
    ...
    fn handles_empty_csv_with_headers() {
        ...
        let chunk = engine
            .execute_query_chunk(
                &registered,
                &format!("SELECT * FROM {}", sql_builder::quote_identifier(&table.table_name)),
                100,
                0,
            )
            .expect("execute query on empty csv");

        assert_eq!(chunk.columns, vec![String::from("id"), String::from("name")]);
    ...
    fn supports_special_header_names() {
        ...
        assert_eq!(chunk.columns, vec![String::from("Customer ID"), String::from("Total Amount($)")]);
        assert_eq!(chunk.rows.len(), 1);
        assert_eq!(chunk.rows[0].get("Customer ID"), Some(&Value::String(String::from("C-1"))));
